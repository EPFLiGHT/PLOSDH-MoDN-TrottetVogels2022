...
from mapping_feature_names import get_name_mapping
...
def plot_heatmap_after_encoder_epoct_all_steps(
    ...
    :return: figure
    """
    df, dict_inverse_mapping = get_name_mapping()
    ...
    feature_mapping = {
        df.loc[index, "name"]: df.loc[index, "varlab"] for index in df.index
    }
    ...
    for index, elem in enumerate(x):
        # x_cleaned[index] = elem.replace('_d0', '').replace('sympcc_', '').replace('sign_', '').replace(
        #     'symp_', '').replace('dem_', '').replace('signv_', '').replace('hist_', '').replace('lab_', '').replace('_o', '')
        x_cleaned[index] = feature_mapping[elem]
    ...
    x_axis.extend(
        [
            ...
            if feature not in dict_inverse_mapping.keys()
            else "    "
            + x_cleaned[index]
            + " : "
            + str(
                dict_inverse_mapping[feature][
                    int(
                        np.around(
                            qst.raw_data.loc[
                                qst.testing_data_indices_reverse_mapping[patients]
