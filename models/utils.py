...
def get_feature_decoders(qst, STATE_SIZE):
    ...
    categorical_feature_decoders = {
        name: EpoctCategoricalDecoder(
            STATE_SIZE, qst.categorical_features_dict[name])
    ...
    for index in range(X.shape[1]):
        ...
        if continuous:
            non_nan_features.append(
                torch.masked_select(
                    X[:, index], ~torch.isnan(X[:, index])).view(-1, 1)
            )
        else:
            non_nan_features.append(
...
def get_auroc(qst, true_targets, probabilities, level):
    disease_mapping = {
    ...
    FPR = np.empty((len(qst.disease_names), n_bootstraps, 40))
    AUC = np.empty((len(qst.disease_names), n_bootstraps))
    pts_mean = np.linspace(0, 1, 40)
    for index, name in enumerate(qst.disease_names):
        probas = probabilities[level, :, index]
        targets = true_targets[:, index]
        for i in range(n_bootstraps):
            indices = rng.randint(0, len(targets), len(targets))
            fpr, tpr, thresh = roc_curve(
                targets[indices], probas[indices], drop_intermediate=False)
            auc = roc_auc_score(targets[indices], probas[indices])
            FPR[index, i, :] = np.interp(pts_mean, fpr, tpr)
            AUC[index, i] = auc
    FPR[:, :, 0] = 0
    FPR[:, :, -1] = 1

    means = np.mean(FPR, axis=1)
    std = np.std(FPR, axis=1)
    means_auc = np.round(np.mean(AUC, axis=1), 2)
    std_auc = np.round(np.std(AUC, axis=1), 2)
    for index, name in enumerate(qst.disease_names):
        plt.plot(pts_mean, means[index, :], label=disease_mapping[name] +
                 ' (AUC = ' + str(means_auc[index]) + '±' + str(std_auc[index]) + ')')
        plt.fill_between(pts_mean, np.clip(means[index, :] +
                                            std[index, :], 0, 1), np.clip(means[index, :] - std[index, :], 0, 1), alpha=0.3)
    ...
    plt.xlim((0,1.01))
    plt.ylim((0,1.01))
    plt.savefig('saved_plots/auroc.pdf')
    return
...
def calibration(qst, disease, level, decoder_output, true_targets, num_intervals):
    ...
    if disease == "all":
        # brier score
        brier_score = torch.empty(len(qst.disease_names))
        brier_score_dict = {name: np.nan for name in qst.disease_names}
        for index, name in enumerate(qst.disease_names):
            probas = decoder_output[level, :, index]
            targets = true_targets[:, index]
            brier_score[index] = torch.mean((probas - targets) ** 2)
            brier_score_dict[name] = torch.mean((probas - targets) ** 2)

        print(
            f'brier scores level {level} {brier_score_dict} and mean {torch.mean(brier_score)}')
        if level == 3:
            get_auroc(qst, true_targets, decoder_output, level)
        # expected calibration errorname:
        ECE = 0
